...
class CxToolCallDTO(BaseDTO):
    ...
    async def _initialize_dto(self, cx_tool_call_item):
        '''Override the base initialization method.'''
        self.id = str(cx_tool_call_item.id)
        await self._process_core_data(cx_tool_call_item)
        await self._process_metadata(cx_tool_call_item)
        await self._initial_validation(cx_tool_call_item)
        self.initialized = True

    async def _process_core_data(self, cx_tool_call_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, cx_tool_call_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, cx_tool_call_item):
