...
class CxConversationDTO(BaseDTO):
    ...
    async def _initialize_dto(self, cx_conversation_item):
        '''Override the base initialization method.'''
        self.id = str(cx_conversation_item.id)
        await self._process_core_data(cx_conversation_item)
        await self._process_metadata(cx_conversation_item)
        await self._initial_validation(cx_conversation_item)
        self.initialized = True

    async def _process_core_data(self, cx_conversation_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, cx_conversation_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, cx_conversation_item):
