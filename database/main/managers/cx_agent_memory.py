# File: database/main/managers/cx_agent_memory.py
from matrx_utils import vcprint


from dataclasses import dataclass
from matrx_orm import BaseManager, BaseDTO
from database.main.models import CxAgentMemory
...
class CxAgentMemoryDTO(BaseDTO):
    ...
    async def _initialize_dto(self, cx_agent_memory_item):
        '''Override the base initialization method.'''
        self.id = str(cx_agent_memory_item.id)
        await self._process_core_data(cx_agent_memory_item)
        await self._process_metadata(cx_agent_memory_item)
        await self._initial_validation(cx_agent_memory_item)
        self.initialized = True

    async def _process_core_data(self, cx_agent_memory_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, cx_agent_memory_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, cx_agent_memory_item):
    ...
    async def get_validated_dict(self):
        ...
        if not validated:
            vcprint(dict_data, "[CxAgentMemoryDTO] Validation Failed", verbose=True, pretty=True, color="red")
        return dict_data
...
class CxAgentMemoryBase(BaseManager[CxAgentMemory]):
    def __init__(self, dto_class: Optional[Type[Any]] = None):
        self.dto_class = dto_class or CxAgentMemoryDTO
        super().__init__(CxAgentMemory, self.dto_class)
