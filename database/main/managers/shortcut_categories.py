...
from database.main.models import ShortcutCategories
...
class ShortcutCategoriesDTO(BaseDTO):
    ...
    async def _initialize_dto(self, shortcut_categories_item):
        '''Override the base initialization method.'''
        self.id = str(shortcut_categories_item.id)
        await self._process_core_data(shortcut_categories_item)
        await self._process_metadata(shortcut_categories_item)
        await self._initial_validation(shortcut_categories_item)
        self.initialized = True

    async def _process_core_data(self, shortcut_categories_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, shortcut_categories_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, shortcut_categories_item):
    ...
    async def get_validated_dict(self):
        ...
        if not validated:
            vcprint(dict_data, "[ShortcutCategoriesDTO] Validation Failed", verbose=True, pretty=True, color="red")
        return dict_data
...
class ShortcutCategoriesBase(BaseManager[ShortcutCategories]):
    def __init__(self, dto_class: Optional[Type[Any]] = None):
        self.dto_class = dto_class or ShortcutCategoriesDTO
        super().__init__(ShortcutCategories, self.dto_class)
