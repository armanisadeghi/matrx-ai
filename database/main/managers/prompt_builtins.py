...
from database.main.models import PromptBuiltins
...
class PromptBuiltinsDTO(BaseDTO):
    ...
    async def _initialize_dto(self, prompt_builtins_item):
        '''Override the base initialization method.'''
        self.id = str(prompt_builtins_item.id)
        await self._process_core_data(prompt_builtins_item)
        await self._process_metadata(prompt_builtins_item)
        await self._initial_validation(prompt_builtins_item)
        self.initialized = True

    async def _process_core_data(self, prompt_builtins_item):
        '''Process core data from the model item.'''
        pass

    async def _process_metadata(self, prompt_builtins_item):
        '''Process metadata from the model item.'''
        pass

    async def _initial_validation(self, prompt_builtins_item):
