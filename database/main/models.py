...
class CxMessages(Model):
    id = UUIDField(primary_key=True, null=False)
    conversation_id = UUIDField(null=False)
    ...
    created_at = DateTimeField(null=False)
    deleted_at = DateTimeField()
    metadata = JSONBField(null=False)
    _inverse_foreign_keys = {}
    _database = "supabase_automation_matrix"
...
class PromptBuiltins(Model):
    ...
    tools = JSONBField()
    settings = JSONBField()
    created_by_user_id = ForeignKey(to_model=Users, to_column='id', )
...
class CxToolCall(Model):
    id = UUIDField(primary_key=True, null=False)
    conversation_id = ForeignKey(to_model=CxConversation, to_column='id', null=False)
    message_id = ForeignKey(to_model=CxMessages, to_column='id', )
    user_id = ForeignKey(to_model=Users, to_column='id', null=False)
...
class CxMessagesManager(BaseManager):
    def __init__(self):
        super().__init__(CxMessages, CxMessagesDTO)
