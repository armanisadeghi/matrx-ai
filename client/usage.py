...
class PricingTier:
    ...
    max_tokens: Optional[int]  # None means unlimited (highest tier)
    ...
    cache_write_5m_price: Optional[float] = None  # 5-minute cache writes
    cache_write_1h_price: Optional[float] = None  # 1-hour cache writes
    cache_hit_price: Optional[float] = None  # Cache hits & refreshes
...
class ModelPricing:
    ...
    def get_tier(self, total_input_tokens: int) -> Optional[PricingTier]:
...
class TokenUsage:
    ...
    def calculate_cost(
        self, pricing_lookup: Optional[Dict[str, ModelPricing]] = None
    ) -> Optional[float]:
