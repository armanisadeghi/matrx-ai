...
in ``ai.ai_requests``.  This guarantees that every execution path —
...
def _require_valid_user_id(user_id: Optional[str], context: str = "") -> str:
...
async def verify_existing_conversation(
    ...
    return row if isinstance(row, dict) else {"id": str(getattr(row, "id", conversation_id))}
