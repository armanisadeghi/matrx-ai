...
async def direct_news_agent_test():
    ...
    # The agent is now scoped to a single conversation.
...
async def test_1B_basic_execution_with_parent_conversation():
    ...
    set_app_context(child_ctx)
    session_with_parent = create_test_session(
...
async def test_4_clone_with_different_models():
    ...
    # 1. First call with original model
    print("=" * 80)
    ...
    # Clone the agent after first call
    print("🔄 Cloning agent...")
    news_agent_clone = news_agent.clone()

    # Apply different model to clone
    ...
    # 2. Second call on ORIGINAL agent (continues with original model)
    print("=" * 80)
    ...
    # 3. Second call on CLONED agent (uses different model)
    print("=" * 80)
